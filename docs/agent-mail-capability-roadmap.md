...
| P1.1 provider implemented | Provider worker, primary-reviewed: guarded enrollment and management adapter | [Backend acceptance](agent-mail-p11-backend-acceptance.md): alpha.5, 298 tests PASS, exact packed installation and real TLS Hub/MCP task, restart and restore PASS |
| P1.2 host integration pending | Management-host and UI work, primary-reviewed: connect enrollment wizard | [DSH rc.2 host gap](agent-mail-management-host-gap.md): establish session authentication, CSRF and profile ACL before mounting the adapter; then verify actual DSH route, directory and explicit task round trip |
| Baseline preserved | Primary agent: retain source and runtime boundaries | Provider increment starts from reviewed public `ca6601c`; archived checkout and deployed provider remain unchanged |
