...
2. Push provider `bdaa0fa` and require remote CI to pass.
3. Annotated tag `v1.0.0-alpha.7` on that commit. GitHub Release (prerelease,
   matching alpha.6) attaching the exact `agent-mail-1.0.0-alpha.7.tgz`,
   `SHA256SUMS`, and SBOM if the owner keeps the alpha.6 asset set. Verify
   anonymous download digest
...
5. Whether the provider Release must include a CycloneDX SBOM as alpha.6 did.
...
10. T1 closeout and T3 SOP were still uncommitted when this file was written.
    T4 must re-read them before publication.
