...
const dshBin = resolveDsh('AgentMemory lifecycle acceptance');
const work = await mkdtemp(path.join(os.tmpdir(), 'dsh-agentmemory-lifecycle.'));

try {
  const store = path.join(work, 'store.json');
  const command = await writeFakeAdapterCommand(path.join(work, 'bin'), store);
  // The wrapper execs Node, so process args keep the unique store path, not the
  // wrapper filename.
  const pattern = store;
  ...
  const env = agentmemoryEnv(command, { DSH_HOME: reconnHome });
  ...
  try {
    ...
    }, 25000, 'initial agentmemory mcp child');
    ...
    const secondPids = await waitFor(async () => {
      ...
      return pids.length > 0 ? pids : null;
    }, 30000, 'reconnected agentmemory mcp child');
  ...
  const installEnv = agentmemoryEnv(command, {
  ...
  assert.match(dumped.stdout, /id: mcp-agentmemory/);
  assert.match(dumped.stdout, /serverName: agentmemory/);
  const removed = await runDsh(
    dshBin,
    ['plugin', '--profile', 'headless', 'remove', '@dff652/dsh-agentmemory'],
    installEnv,
    work,
    120000,
  );
  ...
  assert.doesNotMatch(after.stdout, /id: mcp-agentmemory/);
  assert.deepEqual(await pidsMatching(pattern), []);

  console.log(
    'AgentMemory DSH lifecycle checks: PASS (missing, duplicate namespace, reconnect, cleanup, install/remove)',
  );
} finally {
